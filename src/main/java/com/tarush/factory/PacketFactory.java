...
// Urgent Pointer (2 Bytes)
        ...
        packet[53] = (byte)0x00;
// Payload creation
        String http = "GET /login HTTP/1.1\r\n"+
                "Host: example.com\r\n"+
                "\r\n";
        byte[] httpBytes = http.getBytes();
        byte[] pkt = new byte[packet.length + httpBytes.length];
        System.arraycopy(packet, 0, pkt, 0, packet.length);
        System.arraycopy(httpBytes, 0, pkt, packet.length, httpBytes.length);
        return pkt;
    }
}
